...
class OneYearAgo(object):
    ...
    def compare_days(self, ree_cal, current, past, day, test, correctionOK, correctionKO, result=1):
        ...
        logger.debug("{}? current: {}, past: {}".format(
            test, test_current, test_past))

        if test_current != test_past:
            logger.debug(
                " - Present and past day don't accomplish {}".format(test))
            ...
            if test_current:  # if current accomplish the test // current is_working_day
                if correctionOK:
                    ...
                    # enhacement to return the variable holidays for this year
                    # WIP
                    return ree_cal.get_same_holiday(current, past, day)
                    # return datetime(past.year, current.day(day).month,
                    # current.day(day).day)
            else:
                if correctionKO:
    ...
    def ensure_same_day_scenario(self, current, past, day):
        ...
        past_day = self.compare_days(
            ree_cal, current, past, day, "is_workable", "get_next_workday", "get_next_weekend_day", 1)
        past_day = self.compare_days(
            ree_cal, current, past, day, "is_holiday", None, None, 1)
