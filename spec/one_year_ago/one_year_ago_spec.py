...
with description("One Year Ago"):
    ...
    with context ('can\'t be initialized'):
        with it ('without a valid day to compute'):
            ...
            expect(ini_str).to(raise_error(AssertionError, 'Day must be a datetime object'))
        ...
        with it ('with a None day'):
            ...
            expect(ini_null).to(raise_error(AssertionError, 'Day must be a datetime object'))
    ...
    with context ('must reach the previous date correctly'):
        with it ('for 2016/02/29, 1 year ago was: 2015/02/23'):
            ...
            assert fa_un_any == dia_expected, "The calculated day for {} is not correct: {} (proposal) vs {} (correct)".format(dia,fa_un_any, dia_expected)
        ...
        with it ('for 2016/02/29, 2 year ago was: 2014/02/24'):
            ...
            assert fa_dos_anys == dia_expected, "The calculated day for {} is not correct: {} (proposal) vs {} (correct)".format(dia,fa_dos_anys, dia_expected)
    ...
    with context ('calculated day must ensure mantain same weekday status'):
        with it ('2016/3/23 (Wednesday) -> 2015/3/18 (Wednesday)'):
            ...
            fa_un_any = OneYearAgo(dia).day_year_ago
            assert fa_un_any.isoweekday() == dia.isoweekday(), "Not have the same week day; present: {}({}), past: {}({})".format(dia,dia.isoweekday(), fa_un_any, fa_un_any.isoweekday())
